...
# 데이터 삽입 쿼리
insert_query = '''
INSERT INTO MCS_result_hantoo (loss_probability)
VALUES (%s)
WHERE
'''
...
def change_data_format(row):
    ...
    data["expiration_coupon_rate"] = float(row[16])
    data["kib"] = float(row[17])
    ...
    data["initial_price_evaluation_date"] = row[24].strftime("%Y-%m-%d")
    ...
    data["maturity_date"] = row[30].strftime("%Y-%m-%d")
...
try:
    with connection.cursor() as cursor:
        # SQL 쿼리
        sql = "SELECT * FROM MCS_result_hantoo"
        ...
        update_query = "UPDATE MCS_result_hantoo SET loss_probability = %s WHERE id = %s"
        ...
        for row in result:
            ...
            cursor.execute(update_query, (loss_prob, id))
            connection.commit()
